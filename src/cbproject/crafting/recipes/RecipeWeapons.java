...
public class RecipeWeapons {
	...
	public static void initliazeECRecipes(int pages, String[] additional){
		if(getRecipePages() == 0)
			throw new WrongUsageException("Recipes not loaded while trying to load the EC recipes");
	...
	public static void addECSpecificRecipe(int page, RecipeCrafter... entry){
		if(finished)
			throw new RuntimeException("Trying to add a EC weapon recipe while finished initializing");
		for(RecipeCrafter e: entry){
			recipeEC[page].add(e);
		}
	}
